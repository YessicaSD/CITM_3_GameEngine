...
update_status ModulePhysics3D::Update(float dt)
{
	...
	if(debug == true)
	{
		//Throw balls when you press 1
		if(App->input->GetKey(SDL_SCANCODE_1) == KEY_DOWN)
		{
			Sphere s(1);
			s.SetPos(App->camera0->Position.x, App->camera0->Position.y, App->camera0->Position.z);
			float force = 30.0f;
			AddBody(s)->Push(-(App->camera0->Z.x * force), -(App->camera0->Z.y * force), -(App->camera0->Z.z * force));
		}
	}
...
// ---------------------------------------------------------
PhysBody3D* ModulePhysics3D::AddBody(const Cube& cube, float mass, Module * callback)
{
...
p2DynArray<Cube> ModulePhysics3D::AddRamp( Cube& cub, int radio, float angle)
{
	...
	for (int ang=0; ang < angle; ang += 4)
	{
		...
		vec3 rotVec = { cub.transform[0],cub.transform[1],cub.transform[2] };
		mat4x4 rotationMatrix = rotate(ang, { rotVec.x,rotVec.y, rotVec.z });
		vec3 auxVec = (startPos - circle_center);
		vec4 radiVec = { auxVec.x,auxVec.y,auxVec.z,1 };
		vec4 nextPosDirection = rotationMatrix * radiVec;
		aux.SetPos(nextPosDirection.x + circle_center.x, nextPosDirection.y + circle_center.y, nextPosDirection.z + circle_center.z);
...
PhysVehicle3D* ModulePhysics3D::AddVehicle(const VehicleInfo& info)
{
	...
	btCollisionShape* colShape = new btBoxShape(btVector3(info.chassis.size.x*0.5f, info.chassis.size.y*0.5f, info.chassis.size.z*0.5f));
	shapes.add(colShape);

	btTransform trans;
	trans.setIdentity();
	trans.setOrigin(btVector3(info.chassis.offset.x, info.chassis.offset.y, info.chassis.offset.z));
